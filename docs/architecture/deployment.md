...
| Artefact | Where |
|---|---|
| Service image | `ghcr.io/ensera-ai/taisce:<tag>` and `:<tag without the v>`, also `:latest` and `:<commit sha>` |
| Substrate image | `ghcr.io/ensera-ai/taisce-postgres:<tag>` and `:<tag without the v>`, also `:latest` |
| Helm chart | `oci://ghcr.io/ensera-ai/charts/taisce`, version `<tag without the v>` |

The chart asks for its image by the tag without the `v`, and the release refuses to push a chart
that names a tag it did not push ([D5](../01-decisions.md)).
