...
// Package reportcandidate embeds a bounded question and returns possible retained identities.
// Similarity is a navigation hint only: this package cannot merge, rename or create a report.
...
func (r *Retriever) Search(ctx context.Context, scope string, query Query) (Page, error) {
	...
	result, err := r.store.FindCandidates(ctx, scope, model, vectors[0], pg.ReportCandidateOptions{
