...
func entityEmbeddingsCommand(ctx context.Context, args []string, out io.Writer) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: taisce entity-embeddings <start|build|status|activate|cancel|prune|repair|search> --project <name>")
	}
	operation := args[0]
	switch operation {
	case "start", "build", "status", "activate", "cancel", "prune", "repair", "search":
	default:
		return fmt.Errorf("unknown entity-embeddings operation %q", operation)
	}
	flags := flag.NewFlagSet("entity-embeddings "+operation, flag.ContinueOnError)
	...
	question := flags.String("query", "", "text used to find candidate identities")
	...
	if operation == "search" {
		if err := (entitycandidate.Query{Question: *question, Limit: *limit, Candidates: *candidates}).Validate(); err != nil {
			return err
		}
	}
	...
	default:
		...
		if operation == "start" {
			...
			var current pg.EntityEmbeddingGeneration
			if operation == "search" {
			...
			if operation == "build" {
				...
				result, err = store.Search(ctx, *scope, actor, model, vectors[0], pg.EntityCandidateOptions{
