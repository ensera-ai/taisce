...
// Every invocation does bounded work. Starting and activating are separate operator decisions;
// building a candidate never changes which model answers a read.
func embeddingsCommand(ctx context.Context, args []string, out io.Writer) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: taisce embeddings <start|build|follow|status|activate|cancel|prune|repair|search> --project <name>")
	}
	operation := args[0]
	if operation == "follow" {
		return embeddingFollowCommand(ctx, args[1:], out)
	}
	switch operation {
	case "start", "build", "status", "activate", "cancel", "prune", "repair", "search":
	default:
		return fmt.Errorf("unknown embeddings operation %q", operation)
	}
	flags := flag.NewFlagSet("embeddings "+operation, flag.ContinueOnError)
	...
	candidates := flags.Int("candidates", 0, "opt into approximate search with this candidate bound (maximum 1000)")
	question := flags.String("query", "", "text to search")
	subject := flags.String("subject", "", "optional search subject filter")
	role := flags.String("role", "", "optional search role filter")
	...
	default:
		config, e := inference.EmbeddingConfigFromEnv()
		if e != nil {
			return e
		}
		endpoint, _ := config.EmbeddingsAt()
		digest := sha256.Sum256([]byte(endpoint))
		model := pg.EmbeddingModel{Name: config.EmbeddingModel, Revision: *revision, EndpointHash: hex.EncodeToString(digest[:]), Dimensions: *dimensions}
		embedder := inference.NewEmbedder(config)
		if operation == "start" {
			result, err = store.Start(ctx, *scope, *key, actor, model)
		} else {
			var current pg.EmbeddingGeneration
			if operation == "search" {
			...
			if operation == "build" {
				...
				vectors, e := embedder.Embed(ctx, []string{*question})
				if e != nil {
					return e
				}
				result, err = store.Search(ctx, *scope, actor, model, vectors[0], pg.MessageSearchOptions{Limit: *limit, Candidates: *candidates, Subject: *subject, Role: *role})
			}
		}
	}
	if err != nil {
